...
Your frontend fetches the bootstrap and passes it straight into the browser client, which mounts the Ronan iframe and performs the origin-pinned `postMessage` handshake. See [`@ver-id/embedded-browser-client`](../embedded-browser-client/AUTHENTICATION.md) for the browser side.
