...
const authenticationClient = new VeridEmbeddedAuthenticationClient({ issuerUri, clientId });
...
app.post('/api/verid/start', async (_req, res) => {
  ...
  res.json(bootstrap);
  // { clientId, scope, state, codeChallenge, webhookUri, gatewayUri }
});
```
