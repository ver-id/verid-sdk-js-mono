<template>
  <div class="auth-container">
    ...
    <p class="description">
      This page handles the OAuth callback through the Node.js backend and completes the disclosure flow.
    </p>
...
onMounted(async () => {
  try {
    ...
    if (errorParam) {
      error.value = decodeURIComponent(messageParam || errorParam || 'Disclosure failed');
...
const fetchCallbackInfo = async () => {
  try {
    const response = await fetch(`${API_URL}/disclosure/callback-info`);
...
const finalize = async () => {
  ...
  try {
    const response = await fetch(`${API_URL}/disclosure/finalize`, {
      method: 'POST',
      headers: {
    ...
    // Set disclosure response info
    disclosureResponse.value = data.disclosureResponse;
    tokenResponse.value = JSON.stringify(data.disclosureResponse, null, 2);
    finalizeCodeSnippet.value = data.code;
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Failed to finalize disclosure';
  } finally {
...
const decode = async () => {
  ...
  try {
    const response = await fetch(`${API_URL}/disclosure/decode`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
