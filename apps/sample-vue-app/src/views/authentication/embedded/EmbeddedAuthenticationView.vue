<template>
  <div class="auth-container">
    <h1>Embedded Authentication</h1>
    <p class="description">
      This page demonstrates how to run a Ver.iD authentication flow <strong>inside an iframe on
        your own page</strong>, using @ver-id/embedded-browser-client in the browser and
    ...
    <div class="section step-server">
      ...
      <div
        ...
        class="completed"
      >
        <p>✓ Embedded authentication client initialized on the server</p>
      </div>
      ...
      >
        ...
        <div class="form-group">
          <label for="client_id">Embedded Authentication Flow ID:</label>
          <input
            id="client_id"
      ...
      >
        ...
        <pre>{{ webhookUri }}</pre>
        <p class="note">
          Scope requested: <code>{{ flowScope }}</code> — <strong>a lone <code>openid</code> is
            rejected</strong> by the OAuth server, so authentication always needs a second scope.
        </p>
      </div>
    ...
    >
      ...
      >
        ...
        <div class="form-group">
          ...
          <small>
            Intents allow a custom challenge and brand. The intent is created on the server
            <strong>after</strong> the session, so it can be bound to that session's code challenge.
          </small>
        </div>
    ...
    >
      ...
      <div class="code-example">
        ...
        <pre><code>{{ snippets.fetchBootstrap('authentication') }}</code></pre>
      </div>
    </div>
    ...
    >
      ...
      <div class="lifecycle-note">
        ...
        not a result. It means "the flow finished — go ask your backend", not "here is the ID
        token". The authorization code is delivered to your server on the webhook and never touches
        the browser.
      </div>
    ...
    >
      ...
      <div
        ...
        class="completed"
      >
        <p>✓ Authentication completed and the ID token was decoded on the server</p>
      </div>
      ...
      <div class="code-example">
        <h3>Browser-Side Code:</h3>
        <pre><code>{{ snippets.poll('authentication') }}</code></pre>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <ol>
        ...
        <li>
          ...
          decode the ID token, store it under the state
        </li>
        <li>
...
const defaultFlowId = import.meta.env.VITE_VERID_EMBEDDED_AUTHENTICATION_FLOW_ID || '';
...
const {
  ...
  mountSession,
  startOver,
} = useEmbeddedSession('authentication');
...
const fallbackWebhookNote = `// SERVER — POST /api/authentication/embedded/webhook
//
