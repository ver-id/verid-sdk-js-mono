<template>
  <div class="auth-container">
    ...
    <p class="description">
      This page demonstrates how to use the @ver-id/node-client SDK for OpenID Connect authentication.
    </p>
    ...
    <div>
      ...
      <p v-if="!clientInitialized && !showConfigForm">
        First, initialize the authentication client with your configuration on the Node.js server.
    ...
    <div
      v-if="clientInitialized && !useIntent && !authenticationUrl"
      class="section"
    >
    ...
    >
      ...
      <p>
        Now create an authentication intent on the server using the code challenge. You can optionally provide a custom challenge and brandUuid:
      </p>
    ...
    <div
      ...
      class="section"
    >
      <h2>{{ useIntent ? '5' : '2' }}. Generate Authentication URL</h2>
      ...
      <p v-if="!authenticationUrl">
        Configure the scope and generate the authentication URL with PKCE challenge.
        <br />
        Scopes can be set using environment variables in the node server or passed through the form below.
      </p>
      ...
      >
        ...
        <div class="form-group">
          ...
          <input
            ...
            placeholder="profile"
          >
          <small>Space-separated list of scopes (e.g., "profile email etc.")</small>
        </div>
      </div>
    ...
    <div class="section">
      ...
      <ol>
        <li>
          <strong>Initialize:</strong> Create a VeridAuthenticationClient on the Node.js server with your configuration
        </li>
        <li>
        ...
        <li>
          <strong>Generate URL:</strong> Server generates an authentication URL with PKCE and state parameter (with or without intent)
        </li>
        <li>
...
const generateCodeChallenge = async () => {
  ...
  loading.value = true;
  try {
    const response = await fetch(`${API_URL}/authentication/generate-code-challenge`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
const createIntent = async () => {
  ...
  try {
    ...
    if (intentOptions.value.challenge) payload.challenge = intentOptions.value.challenge;
    if (intentOptions.value.brandUuid) payload.brandUuid = intentOptions.value.brandUuid;

    const response = await fetch(`${API_URL}/authentication/create-intent`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
const generateUrl = async () => {
  ...
  try {
    ...
    if (!data.success) {
      error.value = data.error || 'Failed to generate authentication URL';
      return;
    }

    authenticationUrl.value = data.authenticationUrl;
...
const redirectToAuth = () => {
  if (!authenticationUrl.value) return;
  
  // Redirect to the authentication URL in the same window
  window.location.href = authenticationUrl.value;
};
